...
public class PermissionExpressionNormalizationVisitor implements ExpressionVisitor<Expression> {
    ...
    public Expression visitNotExpression(NotExpression notExpression) {
        ...
        if (inner instanceof CheckExpression) {
            CheckExpression innerCheckExpression = (CheckExpression) inner;
            if (innerCheckExpression.getCheck() instanceof DatastoreEvalFilterExpressionCheck) {
                ((DatastoreEvalFilterExpressionCheck) innerCheckExpression.getCheck()).negate();
                return innerCheckExpression;
            }
        }
